...
case class DataGet(raw_data:String) {
...
object CrossValidationRandomForest {
    def FoldIndexRandomForest(n: Int, k: Int, random_parsed_data: Array[DataGet]): IndexedSeq[Array[DataGet]] = {
      val groupe = n / k
      val j = n % k
  ...
  def main(args: Array[String]): Unit = {
    ...
    val parsed_data = splitIrisData().map { l => DataGet(l) }
    val random_parsed_data = scala.util.Random.shuffle(parsed_data.toSeq).toArray
    val res = FoldIndexRandomForest(random_parsed_data.length, 10, random_parsed_data)
    var cv_score = 0d
    for (i <- res.indices) {
      ...
      val cart_model = {
        ...
        smile.classification.randomForest(x, y, null, 400, 4)
      }
      val threshold = 0.55
      var tp = 0
      var fn = 0
